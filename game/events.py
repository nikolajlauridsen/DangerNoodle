...
def check_keydown_event(event, player, settings):
    if event.key == pygame.K_ESCAPE:
        if settings.game_paused:
            ...
            or event.key == pygame.K_UP and not player.change_y > 0:
        player.go_up()
    elif event.key == pygame.K_s and not player.change_y < 0\
            or event.key == pygame.K_DOWN and not player.change_y < 0:
        player.go_down()
    elif event.key == pygame.K_a and not player.change_x > 0\
            or event.key == pygame.K_LEFT and not player.change_x > 0:
        player.go_left()
    elif event.key == pygame.K_d and not player.change_x < 0\
            or event.key == pygame.K_RIGHT and not player.change_x < 0:
        player.go_right()
